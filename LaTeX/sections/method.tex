...
%Description of what the experiment is trying to achieve
The main objective of this project is to classify the movement of a user's arm based on \acrshort{emg} signals in
4 different classes: static down, moving up, static up, and moving down. This classification is done using a \acrshort{rnn} model
which will be evaluated in terms of accuracy and inference time.
...
kits, various weights from none to six kilograms and a resistance 
...
be more easily reached. Measurement data was acquired using the Digilent Waveforms program with a 
script created with javascript code where data was labeled from 0-3 with each state representing a 
different action, 0 = static down, 1 = moving up, 2 = static up, 3 = moving down. Two LED lights showed what 
movement was expected from the user and the data was collected and labeled accordingly. The test subject, 
the arm, the weight used and what direction the weight was providing resistance in were all recorded. 
The samples were taken from four subjects all of which were taken on the left arm, 53, 33, 19, and 7 of the 
samples were taken from subject one, two, three and four respectively. The final dataset consists on 112 data files
containing samples with a timestamp, the meassurement from each sensor and the label of the sample
from which 71 have 240000 samples and 41 have 480000 samples with a total of 153 minutes of samples. In the Python implementation
subsection \ref{ssec:python} it is explained how the data was split into training and validation sets.
...
of four layers: input layer, first \acrshort{lstm} layer, second \acrshort{lstm} layer, and output layer.

The research by Aviles et al. \cite{RNNEMG} proposed different ranges of values for parameters such as the number of nodes in each one of the \acrshort{lstm} 
...
\begin{figure}[h]
    \centering
    \includegraphics[scale=0.24]{images/rnn_struct.png}
...
Another model was designed as the first model provided only one class as its output. This new model removed the fully connected
...
\begin{figure}[h]
    \centering
    \includegraphics[scale=0.24]{images/rnn_struct_2.png}
...
\subsection{Python Implementation} \label{ssec:python}
...
After the model was trained, the weights were saved both in a .pth file and a .xml file to be used in LabView.
...
initializes all connections, and loads settings and weights. It then runs a loop of data gathering from the \acrshort{FPGA} then running it through the RNN whose output is interpreted into a single 
